...
<script lang="ts">
	...
	const calcYOverflow = (calculatedTop: number) => {
		...
		if (isOverflowing && activator.offsetTop > tooltipHeight) {
			newTop = scrollY + (activator.offsetTop - tooltipHeight);
	...
	const handleActiveUpdate = (_: HTMLElement) => ({
		update: () => {
			if (active) {
				updateTooltipPosition();
			}
